...
public class InvoZone {
    public static ModLogger LOGGER = ModLogger.getLogger(InvoZone.class, new AtomicBoolean(true));
...
//        return new InvoZone(x0, width, y0, height).setXY(new Vector2f(x0, y0)).setWidth(width).setHeight(height);
    ...
    public static InvoZone fromScreenRectangle(ScreenRectangle rectangle) {
        return new InvoZone(rectangle.left(), rectangle.width(), rectangle.top(), rectangle.height());
    }
    ...
    public Vector2f rotateAroundPoint(Vector2f origin, ANCHORPOINT anchorPoint, float rotation){
...
//        p'y = sin(theta) * (px-ox) + cos(theta) * (py-oy) + oy
    ...
    public ScreenRectangle rect(boolean roundSizeUp){
        if (roundSizeUp) return new ScreenRectangle((int) this.x(), (int) this.y(), (int)Math.ceil(this.width), (int) Math.ceil(this.height));
...
//        this.center(originalZone);
    ...
    public void deserializeNBT(CompoundTag tag){
        ...
        String[] stringArray = tag.getString(DIMENSION_FLOATS).get().split(":");
